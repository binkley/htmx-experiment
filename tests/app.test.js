...
describe('Advanced HTMX Todo API', () => {
    ...
    it('GET / should return the initial HTML document', async () => {
        ...
        expect(res.text).toContain('EJS + HTMX Demo');
    ...
    it('GET /todos should return an empty list and OOB stats initially', async () => {
        ...
        expect(res.status).toBe(200);
        expect(res.text).toContain('id="todo-stats"');
        expect(res.text).toContain('Total:</strong> 0');
    });

    it('GET /todos?q=... should filter results', async () => {
        await request(app).post('/todos').send('title=Apple').set('Content-Type', 'application/x-www-form-urlencoded');
        await request(app).post('/todos').send('title=Banana').set('Content-Type', 'application/x-www-form-urlencoded');
    ...
    it('POST /todos should create a new todo and return HTML with OOB stats', async () => {
        ...
        expect(res.text).toContain('id="todo-stats"');
        expect(res.text).toContain('Total:</strong> 1');
    });

    it('POST /todos should return 400 if title is empty', async () => {
        const res = await request(app).post('/todos').send('title=   ').set('Content-Type', 'application/x-www-form-urlencoded');
        expect(res.status).toBe(400);
        
        const res2 = await request(app).post('/todos').send('').set('Content-Type', 'application/x-www-form-urlencoded');
        expect(res2.status).toBe(400);
    });

    it('GET /todos/:id/edit should return the edit form', async () => {
        await request(app).post('/todos').send('title=To Edit').set('Content-Type', 'application/x-www-form-urlencoded');
    ...
    it('GET /todos/:id should return a single todo item', async () => {
        await request(app).post('/todos').send('title=Single Item').set('Content-Type', 'application/x-www-form-urlencoded');
    ...
    it('PUT /todos/:id should update title and return the item', async () => {
        await request(app).post('/todos').send('title=Old Title').set('Content-Type', 'application/x-www-form-urlencoded');
        const res = await request(app).put('/todos/1').send('title=New Title').set('Content-Type', 'application/x-www-form-urlencoded');
        expect(res.status).toBe(200);
        expect(res.text).toContain('New Title');
        
        const res404 = await request(app).put('/todos/999').send('title=New Title').set('Content-Type', 'application/x-www-form-urlencoded');
        expect(res404.status).toBe(404);
    });
    
    it('PUT /todos/:id should ignore empty title update', async () => {
        await request(app).post('/todos').send('title=Old Title').set('Content-Type', 'application/x-www-form-urlencoded');
        const res = await request(app).put('/todos/1').send('title=  ').set('Content-Type', 'application/x-www-form-urlencoded');
        expect(res.status).toBe(200);
    ...
    it('PATCH /todos/:id/toggle should toggle completion state and return OOB stats', async () => {
        await request(app).post('/todos').send('title=To toggle').set('Content-Type', 'application/x-www-form-urlencoded');
        
        let res = await request(app).patch('/todos/1/toggle');
        expect(res.status).toBe(200);
        expect(res.text).toContain('checked');
        expect(res.text).toContain('Completed:</strong> 1');
        
        res = await request(app).patch('/todos/1/toggle');
        expect(res.status).toBe(200);
        expect(res.text).not.toContain('checked');
        expect(res.text).toContain('Completed:</strong> 0');
    ...
    it('DELETE /todos/:id should remove the todo and return OOB stats', async () => {
        await request(app).post('/todos').send('title=To delete').set('Content-Type', 'application/x-www-form-urlencoded');
